          ...
          <q-card>
            ...
            <q-card-section class="flex flex-center" >
               ...
               <div class="row">
                ...
                <div class="col">
                  {{props.row.gender ? $t('male'):$t('female')}}
                </div>
               </div>
...
export default {
    ...
    return {
      ...
      columns: [
        ...
        { name: 'number', align: 'center', label: this.$t('number'), field: 'number', sortable: true },
        { name: 'birthday', align: 'center', label: this.$t('birthday'), field: 'birthday', sortable: true },
        { name: 'fromdate', align: 'right', label: this.$t('from_date'), field: 'fromdate', sortable: true },
        { name: 'gender', align: 'right', label: this.$t('gender'), field: 'gender', sortable: true }
      ],
      data: [
  ...
  methods: {
    initialize(){
      this.$axios.get('/patient_request')
      .then(response => {
    ...
    async removePatient(id){
      this.$q.dialog({
        ...
        let response = await this.$helper.deleteInstance({id: id}, '/patient_request')
        if (response == true){
          this.$q.notify({
